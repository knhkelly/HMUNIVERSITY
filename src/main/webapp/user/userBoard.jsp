...
<%
	ArrayList<BoardBean> articleList = (ArrayList<BoardBean>)request.getAttribute("allBoard");
...
<%-- 					<a href="boardList.board?page=<%=a %>">[<%=a %>]</a>&nbsp; --%>
					<button class="btn btn-primary" onclick="loadNoticePageList(<%=a %>)"><%=a %></button>&nbsp;
				<%} %>
		<%} %>
...
<%-- 			<a href="boardList.board?page=<%=nowPage+1 %>">[다음]</a> --%>
			<button class="btn btn-primary" onclick="loadNoticePageList(<%=nowPage+1 %>)">다음</button>
		<%} %>
	</section>
<% } else{ %>
